...
(user_id, session_id, session_date, session_tz, earliest, latest)
VALUES (?, ?, ?, ?, ?, ?)"
...
create_session_db = <<-SQL
  CREATE TABLE IF NOT EXISTS user_schedule(
    ...
    PRIMARY KEY (user_id, session_id, session_date, partner_id),
...
#loads users table with test user data
25.times do
  insert_into_table(db, user_insert_string, Faker::Name.first_name, Faker::Name.last_name, utc_offset.keys[rand(utc_offset.length)]) 
end
...
session_number.each do |sess|
  ...
  user_recs.each do |id|
    ...
    # create 3 records per user record per assignment
    3.times do
      ...
      #increments dates to ensure that the session dates are different
      sess_date = sess_date + add_days(0,rand(2) + 1)
      
      #check for daylight savings
      utc_adjust = sess_date.dst? ? utc_offset[id['user_tz']] - 1 : utc_offset[id['user_tz']]

      #The early and latest values represents the availability of the user
  ...
  #the first number of the session contains the week number. If the week number has changed 
  #increment by 1 week.
  sess_week = sess.split(".")[0]
  if sess_week != comp_week
    pref_date = pref_date + add_days(1,0) #add 7 days
    comp_week = sess_week
  end
end
